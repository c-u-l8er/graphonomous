...
## The code (G0-A/G0-B 2026-09-02 · G0-B.1, G0-E, G0-C spike 2026-09-03)
...
lib/wrl_world.mjs    the V2-shaped semantic world, kernel rev-, G0 grelpre- (provisional allocation preimage, GAP-W11 — never a rel-), gsem- — PROVISIONAL (D-036/D-037) (G0-C spike)
...
projections/         baseline + historical (each: records, cas, derived/, world/) · pre-b1/ and pre-d034/ receipts (superseded, preserved) · EVIDENCE.md
handoff/WRL_SCHEMA_OR_PROFILE/graphonomous.semantic.v0.json  the profile as data (PROPOSED)
test/                node --test; canon_twin.py is the independent Python twin; helpers/fake_repo.mjs for synthetic sources
tools/               evidence.py (regenerates EVIDENCE.md) · make_gpt_bundle.mjs (handoff ZIP + REPRO_DEPENDENCIES)
```
...
`node bin/g0.mjs world --dir projections/baseline`. Zero runtime dependencies; the TRVM encoder/CAS and the WRL relation
kernel are imported from the pinned sibling checkouts and their blob OIDs are checked at test time. The tests that need
